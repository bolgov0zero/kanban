...
switch ($action) {
	...
	case 'add_task':
		...
		if (!empty($bot_token) && !empty($chat_id)) {
			...
			$text = "📝 <b>Новая задача добавлена</b>\n👤 <i>Кем:</i> $user_name\n📋 <i>Задача:</i> $title\n🧑‍💻 <i>Исполнитель:</i> $resp_name";
	...
	case 'move_task':
		...
		$col = $db->querySingle("SELECT auto_complete, timer FROM columns WHERE id=$cid", true);
		...
		$title = $task_data['title'] ?? 'Без названия';
		$col_name = $db->querySingle("SELECT name FROM columns WHERE id=$cid");
		$resp = $task_data['responsible'] ?? 'Не указан';
		$resp_name = $db->querySingle("SELECT name FROM users WHERE username='$resp'", true)['name'] ?? $resp;
		$auto = $col['auto_complete'] ? 'автоматически завершена' : 'перемещена';
		$text = "🔄 <b>Задача $auto</b>\n👤 <i>Кем:</i> $user_name\n📋 <i>Задача:</i> $title\n📂 <i>В колонку:</i> $col_name\n🧑‍💻 <i>Исполнитель:</i> $resp_name";
		if (!empty($bot_token) && !empty($chat_id)) {
			sendTelegram($bot_token, $chat_id, $text);
		}
		break;
